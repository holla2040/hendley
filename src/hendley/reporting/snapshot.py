...
what each BOM line resolved to at emit time — chosen ref, rank used,
...
def write_release_snapshot(
    ...
    output or equivalent), embedded verbatim. Never overwrites: a same-second
    emit gets a ``-2``/``-3`` suffix (each emit is its own fact).
    """
    ...
    populated = [x for x in lines if not x.get("dnp")]
    doc = {
        ...
        "summary": {
            ...
            "dnpLines": len(lines) - len(populated),
        },
