...
from hendley.reporting.snapshot import snapshot_path, write_release_snapshot
...
def test_snapshot_summary_counts_dnp_lines(tmp_path):
    resolution = {
        "design": "comet", "productionQuantity": 5,
        "lines": [
            {"designators": ["R1"], "lcsc": "C2", "quantityPer": 1},
            {"designators": ["TP1"], "dnp": True, "quantityPer": 1},
        ],
    }
    doc = json.loads(write_release_snapshot(
        resolution, tmp_path / "b.csv", when="2026-07-10T06:30:00+00:00").read_text())
    assert doc["summary"] == {"lines": 2, "partsPerBoard": 1, "substitutions": 0,
                              "dnpLines": 1}
